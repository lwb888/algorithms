...
public class BinarySearch {
    ...
    public static int firstBiggerThanOrEqual(int[] arr, int data) {
        if (arr == null || arr.length == 0) return -1;
    ...
    public static int lastSmallerThanOrEqual(int[] arr, int data) {
        if (arr == null || arr.length == 0) return -1;
        int low = 0, high = arr.length - 1;
    ...
    public static void main(String[] args) {
        ...
        int[] arr2 = {3,4,6,8,9,10};
        System.out.println(sol.firstBiggerThanOrEqual(arr2, 7));

        System.out.println(sol.lastSmallerThanOrEqual(arr, 8));
    }
}
